...
def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k):
    ...
    plt.axis('equal')
    # plt.savefig('./DVL/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}.png'
...
if __name__ == '__main__':
    ...
    for lr_len in np.arange(2 ,3.5, 0.5, dtype='float64'):
        for angle in np.arange(20,70, 10, dtype='float64'):
            for comp_ud_len in np.arange(-1, 1, 0.4):
                for low_amplitude in np.arange(0.7, 1.6 ,0.4,dtype='float64'):
                    for lowax in np.arange(0.3, 0.4, 0.1, dtype='float64'):
                            for up_amplitude in np.arange(0.5, 1.1, 0.5, dtype='float64'):
                                for upax in np.arange(0.4, 0.6, 0.1, dtype='float64'):
                                    for k in np.arange(0, 0.1, 0.1, dtype='float64'):
