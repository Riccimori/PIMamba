...
def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k):
    ...
    plt.axis('equal')
    # plt.savefig('./SVL/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}.png'
    #             .format(lr_lengths, angle_Du, up_down_complen, lowamplitude, lowax, k))
    plt.show()
    plt.close()
    ...
    for lr_len in np.arange(2, 3.5, 0.5, dtype='float64'):
        for angle in np.arange(20, 70, 10, dtype='float64'):
            for comp_ud_len in np.arange(-1, 1, 0.4):
                        for up_amplitude in np.arange(0, 0.2, 1,dtype='float64'):
                            for low_amplitude in np.arange(0.7, 2, 0.3, dtype='float64'):
