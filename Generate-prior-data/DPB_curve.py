...
'''
Down Pump Bumping
We use a total of 13 range parameters.
...
c, d : Horizontal and vertical shift of the lower left vertex
'''


def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k,c,d):

    plt.figure(figsize=(2.24, 2.24),facecolor='black')

    # Lower Curve
    x0, y0 = 0, 0
    angle1 = np.radians(angle_Du)
    x0loss_start = x0 - d
    x0loss_end = x0 + d * np.sin(angle1)
    y0loss_start = y0 - c * np.sin(angle1)
    x1 = x0 + ud_lengths
    y1 = lowamplitude * np.cos(lowpianyi + lowax * x1)
    x1_point = np.linspace(x0loss_end, x1, 50)
    y1_point = lowamplitude * np.cos(lowpianyi + lowax * x1_point)
    plt.plot(x1_point, y1_point, 'white')
    y1cos_qian = lowamplitude * np.cos(lowpianyi + lowax * x0loss_end)
    y1cos_hou = lowamplitude * np.cos(lowpianyi + lowax * x1)

    # Lower curve is prominent
    plt.plot([x0loss_start, x0loss_end], [y0loss_start, y1cos_qian], 'white')
    ...
    x4_start = x0loss_start
    x4_end = x3
    y4_start = y0loss_start
    y4_end = y3sin_qian
    ...
    plt.axis('equal')
    # plt.savefig('./DPB/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.2f}-{:.1f}-{:.1f}.png'
    #             .format(lr_lengths, angle_Du, up_down_complen, lowamplitude, upamplitude, k,c,d))
    plt.show()
    plt.close()

if __name__ == '__main__':
    ud_len = 5
    for lr_len in np.arange(2, 3.5, 0.5, dtype='float64'):
        for angle in np.arange(10, 60, 10):
            for comp_ud_len in np.arange(0, 0.8, 0.2):
                for low_amplitude in np.arange(0, 0.2, 0.1, dtype='float64'):
                    for up_amplitude in np.arange(0, 0.2, 0.1, dtype='float64'):
                        for k in np.arange(0, 0.2, 0.05, dtype='float64'):
                            for c in np.arange(1, 3, 1):
                                for d in np.arange(0.5, 1, 0.4):
                                    plot_parallelogram(ud_lengths=ud_len, lr_lengths=lr_len, up_down_complen=comp_ud_len,
                                                       upamplitude=up_amplitude,upax=1,uppianyi=0,
                                                       lowamplitude=low_amplitude, lowax=1,lowpianyi=0,angle_Du=angle, k=k,c=c, d=d)