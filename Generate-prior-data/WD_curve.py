...
def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k):
    ...
    plt.axis('equal')
    # plt.savefig('./WD/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}.png'
...
if __name__ == '__main__':
    ud_len = 5
    for lr_len in np.arange(2 ,4.5 , 0.5, dtype='float64'):
        for angle in np.arange(10 ,60, 10, dtype='float64'):
            for comp_ud_len in np.arange(0, 1, 0.2, dtype='float64'):
                for low_amplitude in np.arange(0.1, 0.3 , 0.1, dtype='float64'):
                    for up_amplitude in np.arange(0.1, 0.3 ,0.1, dtype='float64'):
                        for lowax in np.arange(3, 5, 1, dtype='float64'):
                            for upax in np.arange(3, 5, 1, dtype='float64'):
