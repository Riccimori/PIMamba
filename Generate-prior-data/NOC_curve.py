...
def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k):
    ...
    plt.axis('equal')
    # plt.savefig('./NOC/zuoping-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}.png'
...
if __name__ == '__main__':
    ...
    for lr_len in np.arange(2 ,4.5 , 0.5, dtype='float64'):
        for angle in np.arange(10 ,60, 5, dtype='float64'):
            for comp_ud_len in np.arange(0, 1, 0.2):
                for amplitude in np.arange(0, 0.2 ,0.1,dtype='float64'):
                    for ax in np.arange(0.5, 1.1,0.5, dtype='float64'):
                        for k in np.arange(0, 0.2, 0.1, dtype='float64'):
                            plot_parallelogram(ud_lengths=5, lr_lengths=lr_len, up_down_complen=comp_ud_len,
                                                            lowamplitude=amplitude, lowax=0.5,lowpianyi=0,
                                                            upamplitude=amplitude,upax=0.5,uppianyi=0,
                                                           angle_Du=angle, k=k)