...
def plot_parallelogram(ud_lengths, lr_lengths,up_down_complen,midx_lengths,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k, lowx_lengths,lowy_lengths):
    ...
    # Lower right curve
    x2_rightlow = x1 + midx_lengths/5
    y2_rigthlow = lowy_lengths
    plt.plot([x1,x2_rightlow],[y1cos_hou,y2_rigthlow],'white')
    # Right middle curve
    x2_rightmid = x2_rightlow + 4*(midx_lengths/5)
    ...
    plt.axis('equal')
    # plt.savefig('./ILS/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}.png'
                    ...
                    for y_po_len in np.arange(y_po/3, 4*y_po/5, y_po/4, dtype='float64'):
                        for amplitude in np.arange(0, 0.1 ,0.1,dtype='float64'):
                            for k in np.arange(0, 0.2, 0.1, dtype='float64'):
                                ...
                                for midx_len in np.arange(midx/2, midx, 1):
