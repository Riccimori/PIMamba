...
def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k):
    ...
    plt.axis('equal')
    # plt.savefig('./TVL/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.2f}-{:.1f}-{:.1f}.png'
    #             .format(lr_lengths, angle_Du, up_down_complen, upamplitude, upax, uppianyi, k))
    plt.show()
    plt.close()
    ...
    for lr_len in np.arange(2, 3.5, 0.5, dtype='float64'):
        for angle in np.arange(20 ,70, 10, dtype='float64'):
            for comp_ud_len in np.arange(0, 1, 0.2):
                        for low_amplitude in np.arange(0, 0.2, 1, dtype='float64'):
                            for up_amplitude in np.arange(0.5, 1.5, 0.2, dtype='float64'):
                                for upax in np.arange(0.4, 0.5, 0.05, dtype='float64'):
                                            for k in np.arange(0, 0.1, 1, dtype='float64'):
                                                for uppianyi in np.arange(0, 0.2, 0.1, dtype='float64'):
                                                    plot_parallelogram(ud_lengths=ud_len, lr_lengths=lr_len, up_down_complen=comp_ud_len,
                                                                        lowamplitude=low_amplitude, lowax=1,lowpianyi=0,
                                                                        upamplitude=up_amplitude,upax=upax,uppianyi=uppianyi,
                                                                       angle_Du=angle, k=k)