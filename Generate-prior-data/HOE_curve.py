...
def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k):
    ...
    plt.axis('equal')
    # plt.savefig('./HOE/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}.png'
    #             .format(lr_lengths, angle_Du, up_down_complen, lowamplitude,lowax, upamplitude, upax,k))
    plt.show()
    plt.close()
...
if __name__ == '__main__':
    ud_len = 5
    for lr_len in np.arange(4, 8 , 1, dtype='float64'):
        for angle in np.arange(25, 30, 10, dtype='float64'):
            for comp_ud_len in np.arange(-1, 1, 0.9):
                for low_amplitude in np.arange(0.4, 0.6 ,0.1,dtype='float64'):
                    for lowax in np.arange(0.4, 0.7, 0.1, dtype='float64'):
                            for up_amplitude in np.arange(0.3, 0.6, 0.1, dtype='float64'):
                                for upax in np.arange(0.4, 0.7, 0.1, dtype='float64'):
                                    for k in np.arange(0.2, 0.4, 0.1, dtype='float64'):
                                            plot_parallelogram(ud_lengths=ud_len, lr_lengths=lr_len, up_down_complen=comp_ud_len,
                                                                lowamplitude=low_amplitude, lowax=lowax,lowpianyi=2,
