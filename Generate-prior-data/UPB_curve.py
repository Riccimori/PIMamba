...
a, b : Horizontal and vertical shift of the upper right vertex
'''


def plot_parallelogram(ud_lengths, lr_lengths, up_down_complen,lowamplitude,lowax,lowpianyi,upamplitude,upax,uppianyi,angle_Du,k,a,b):
    ...
    # upper curve is prominent
    angle1 = np.radians(angle_Du)
    x3loss_start = x2 - a
    x3loss_end = x2 + a * np.sin(angle1)
    y3loss_end = y2 + b * np.sin(angle1)

    # Upper curve
    x3 = x2 - ud_lengths + up_down_complen
    y3 = y2
    x3_point = np.linspace(x3, x3loss_start, 50)
    y3_point = y3 + upamplitude * np.sin(uppianyi + upax * x3_point)

    plt.plot(x3_point, y3_point, 'white')
    y3sin_hou = y3 + upamplitude * np.sin(uppianyi + upax * x3loss_start)
    y3sin_qian = y3 + upamplitude * np.sin(uppianyi + upax * x3)

    plt.plot([x3loss_start, x3loss_end], [y3sin_hou, y3loss_end], 'white')
    ...
    plt.axis('equal')
    # plt.savefig('./UPB/z-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.1f}-{:.2f}-{:.1f}-{:.1f}.png'
    #             .format(lr_lengths, angle_Du, up_down_complen, lowamplitude, upamplitude, k,a,b))
    plt.show()
    plt.close()
...
if __name__ == '__main__':
    ...
    for lr_len in np.arange(2, 3.5, 0.5, dtype='float64'):
        for angle in np.arange(10, 50, 10):
                ...
                for low_amplitude in np.arange(0, 0.2, 0.1, dtype='float64'):
                    for up_amplitude in np.arange(0, 0.2, 0.1, dtype='float64'):
                        for k in np.arange(0, 0.2, 0.1, dtype='float64'):
                            for a in np.arange(0.5, 1, 0.4):
                                for b in np.arange(1, 3, 1):
                                    plot_parallelogram(ud_lengths=x_down_len, lr_lengths=lr_len, up_down_complen=comp_ud_len,
                                                       upamplitude=up_amplitude,upax=1,uppianyi=0,
                                                       lowamplitude=low_amplitude, lowax=1,lowpianyi=0,angle_Du=angle, k=k,a=a, b=b)
